...
async def get_stations_last_prices(
    ...
    # Etapa de match con los filtros
    match_stage = {}
    ...
    if product or product_id:
        ...
        if product_match:
            match_stage["$and"] = [product_match]
    
    # Añadir la etapa de match al pipeline si hay filtros
    if match_stage:
    ...
    stations = collection_name.aggregate(pipeline)
    return list_serial(stations)
